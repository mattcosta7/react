...
struct OutlinedJsxAttribute {
    ...
    /// Location of `original_name` in source. `new_name` is compiler-generated
    /// and so has no source location of its own.
    original_name_loc: Option<react_compiler_diagnostics::SourceLocation>,
    new_name: String,
    place: Place,
}
            ...
            if let Some(kids) = children {
                for child in kids {
                    ...
                    attributes.push(OutlinedJsxAttribute {
                        ...
                        // Synthesized from a promoted temporary for a JSX
                        // child, so there is no source name to point at.
                        original_name_loc: None,
                        new_name,
                        place: child.clone(),
                    });
                }
...
fn emit_outlined_jsx(
    ...
    let props: Vec<JsxAttribute> = outlined_props
        ...
        .map(|p| JsxAttribute::Attribute {
            ...
            // `new_name` is generated by the compiler and does not appear in
            // source, so it stays unmapped.
            name_loc: None,
            place: p.place.clone(),
        })
        .collect();
        ...
        } = &instr.value
        {
            ...
            for prop in props {
                ...
                new_props.push(JsxAttribute::Attribute {
                    ...
                    name_loc: new_prop.original_name_loc,
    ...
    for old_prop in old_props {
        ...
        old_to_new.insert(
            ...
            OutlinedJsxAttribute {
                ...
                original_name_loc: old_prop.original_name_loc,
